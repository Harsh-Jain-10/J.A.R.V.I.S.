...
def get_due_reminders() -> list[dict]:
    """Return untriggered reminders due within the last 10 minutes.

    Reminders overdue by more than 10 minutes are considered 'stale' —
    they should NOT fire on startup (they would have been missed while JARVIS
    was offline). Call dismiss_stale_reminders() at startup to clean them up.
    """
    now = datetime.now()
    now_iso = now.isoformat(timespec="seconds")
    # Only return reminders due in the last 10 minutes (not ancient ones)
    ...
    try:
        rows = conn.execute(
            ...
            "WHERE triggered = 0 AND datetime <= ? AND datetime >= ?",
            (now_iso, cutoff),
        ).fetchall()
        return [dict(r) for r in rows]
    except Exception as exc:
...
def dismiss_stale_reminders() -> int:
    """Mark all overdue reminders (older than 10 minutes) as triggered.

    Call this once at startup so old reminders from previous sessions don't
    all fire simultaneously when JARVIS restarts.
    Returns the number of dismissed reminders.
    """
    from datetime import timedelta
    ...
    try:
        cur = conn.execute(
            "UPDATE reminders SET triggered = 1 "
        ...
        count = cur.rowcount
        if count:
            logger.info("Dismissed %d stale overdue reminder(s) from previous session.", count)
        return count
    except Exception as exc:
        logger.error("dismiss_stale_reminders error: %s", exc)
        return 0
    finally:
        conn.close()
