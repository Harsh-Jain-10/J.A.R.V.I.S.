    ...
    try:
        ...
        for r in upcoming:
            try:
                ...
                if dt.date() == today:
                    date_prefix = "Today at "
                elif dt.date() == today + timedelta(days=1):
                    date_prefix = "Tomorrow at "
                else:
                    date_prefix = dt.strftime("%a at ")
                display_time = f"{date_prefix}{time_str}"
            except Exception:
                display_time = r['datetime']
        ...
        ui_bridge.show_reminders_card(formatted)
    except Exception as bridge_exc:
        logger.warning("Could not show reminders card: %s", bridge_exc)
