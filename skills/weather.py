    ...
    try:
        ...
        try:
            ...
            card_data = {
                ...
                "feels": int(feels_c),
                "condition": desc,
                "icon": _get_weather_emoji(data["weather"][0]["icon"]),
                "humidity": humidity,
                "wind": wind_kmh,
                "visibility": f"{round(data.get('visibility', 10000) / 1000, 1)} km",
                "forecast": []
            }
            # Fetch forecast to populate forecast hourly data on the card
            try:
                forecast_url = f"{_BASE_URL}/forecast"
                forecast_params = {"q": city, "appid": OPENWEATHER_API_KEY, "cnt": 5}
                f_resp = requests.get(forecast_url, params=forecast_params, timeout=5)
                if f_resp.status_code == 200:
                    ...
                    for item in f_data.get("list", []):
                        ...
                        forecast_list.append({
                            "label": label,
                            "temp": int(_kelvin_to_celsius(item["main"]["temp"]))
                        })
                    card_data["forecast"] = forecast_list
            except Exception as f_exc:
                logger.debug("Failed to fetch forecast list for card: %s", f_exc)
            ui_bridge.show_weather_card(card_data)
        except Exception as bridge_exc:
            logger.warning("Could not show weather card: %s", bridge_exc)
